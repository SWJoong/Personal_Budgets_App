/**
 * SIS-A (지원요구척도 성인용) 계산 유틸
 * GAS_Code.js의 getStandardScore / getIndexAndPercentile 로직을 TypeScript로 포팅
 */

export type SisSubScale = '2A' | '2B' | '2C' | '2D' | '2E' | '2F'

export const SIS_SUB_SCALES: { key: SisSubScale; label: string }[] = [
  { key: '2A', label: '가정 생활' },
  { key: '2B', label: '지역사회 생활' },
  { key: '2C', label: '평생학습' },
  { key: '2D', label: '고용' },
  { key: '2E', label: '건강 & 안전' },
  { key: '2F', label: '사회' },
...
/** 원점수 → 표준점수 변환 테이블
 *  각 행: { s: 표준점수, r: [2A범위, 2B범위, 2C범위, 2D범위, 2E범위, 2F범위] }
 *  -1 = 해당 범위 없음
 */
const RANGES: { s: number; r: [number, number][] }[] = [
...
const IDX_MAP: Record<SisSubScale, number> = { '2A': 0, '2B': 1, '2C': 2, '2D': 3, '2E': 4, '2F': 5 }

/** 원점수 → 표준점수 (1~20) */
export function getStandardScore(scale: SisSubScale, rawScore: number): number {
  const idx = IDX_MAP[scale]
  for (const row of RANGES) {
    const [min, max] = row.r[idx]
    if (min === -1) continue
    if (rawScore >= min && rawScore <= max) return row.s
  }
...
/** 표준점수 합계 → 지원요구지수 + 백분위 */
export function getIndexAndPercentile(total: number): { indexScore: string; percentile: string } {
...
/** 6개 원점수를 한번에 계산해 결과 반환 */
export function calculateSisA(raw: Record<SisSubScale, number>) {
  const std: Record<SisSubScale, number> = {
    '2A': getStandardScore('2A', raw['2A']),
    '2B': getStandardScore('2B', raw['2B']),
    '2C': getStandardScore('2C', raw['2C']),
    '2D': getStandardScore('2D', raw['2D']),
    '2E': getStandardScore('2E', raw['2E']),
    '2F': getStandardScore('2F', raw['2F']),
  }
  const totalStd = std['2A'] + std['2B'] + std['2C'] + std['2D'] + std['2E'] + std['2F']
  const { indexScore, percentile } = getIndexAndPercentile(totalStd)
  return { std, totalStd, indexScore, percentile }
}
