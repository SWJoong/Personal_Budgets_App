...
const STATUS_STYLE: Record<AxisStatus, { badge: string; emoji: string }> = {
  ok: { badge: 'bg-success-bg text-success-fg ring-success-fg/20', emoji: '✅' },
      ...
      <main id="main-content" tabIndex={-1} className="flex-1 w-full max-w-lg mx-auto p-4 sm:p-6 flex flex-col gap-4">
        ...
        <div className="flex gap-2">
          <div className="flex-1 p-3 rounded-2xl bg-muted ring-1 ring-border">
            <div className="text-xs text-muted-foreground">사정한 욕구</div>
        ...
        <ul className="flex flex-col gap-2">
          {rows.map((r) => {
            ...
            return (
              <li key={r.domainId} className="p-4 rounded-2xl bg-card ring-1 ring-border flex flex-col gap-2">
                ...
                <div className="flex flex-wrap gap-x-4 gap-y-1 text-sm text-muted-foreground">
                  <span>
                    <span className="text-muted-foreground">욕구 </span>
                    {r.needsCount}건
                  </span>
                  <span>
