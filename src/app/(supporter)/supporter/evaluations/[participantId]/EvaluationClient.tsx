  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <section className="p-5 rounded-2xl bg-white ring-1 ring-zinc-200 flex flex-col gap-4">
        <h2 className="text-sm font-black text-zinc-400 uppercase tracking-widest">새 모니터링 기록</h2>

        <fieldset className="flex flex-col gap-2">
          <legend className="text-sm font-bold text-zinc-700 mb-2">어떻게 확인했나요?</legend>
            ...
            {METHODS.map((m) => (
              <button
                ...
                className={`flex flex-col items-center gap-1 p-3 rounded-xl ring-2 transition-all ${
                  method === m.value
                    ? 'ring-zinc-900 bg-zinc-50 font-black'
        ...
        <FormField id="observed-change" label="실무자 관찰 (내가 본 변화)">
          {(field) => (
            <textarea
              ...
              rows={3}
              className="p-3 rounded-xl bg-zinc-50 ring-1 ring-zinc-200 text-sm focus:ring-zinc-400 focus:outline-none resize-y"
            />
          )}
        </FormField>
      ...
      <section className="flex flex-col gap-3">
        <h2 className="text-sm font-black text-zinc-400 uppercase tracking-widest">기록 타임라인</h2>
        {timeline.length === 0 ? (
          <div className="p-8 rounded-2xl bg-zinc-100 text-center">
            ...
            {timeline.map((entry) => (
              <li key={`${entry.kind}-${entry.id}`}>
                {entry.kind === 'monitoring' && entry.monitoring && (
                  <div className="p-4 rounded-2xl bg-white ring-1 ring-zinc-200 flex flex-col gap-2">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-xs text-zinc-400">{entry.monitoring.monitoringDate}</span>
                    </div>
                    {entry.monitoring.observedChange && (
                      <p className="text-sm text-zinc-700 leading-relaxed">
                        <span className="font-bold text-zinc-500">관찰 </span>
                        {entry.monitoring.observedChange}
                      </p>
                    )}
                    {entry.monitoring.participantVoice && (
                      <p className="text-sm text-zinc-700 leading-relaxed">
                        <span className="font-bold text-zinc-500">당사자 말 </span>
                        {entry.monitoring.participantVoice}
                      </p>
                    )}
                    {!entry.monitoring.allocationId && (
                      <span className="text-[11px] text-zinc-400">배정 전 기록</span>
                    )}
                  </div>
                )}
                ...
                {entry.kind === 'settlement' && entry.settlement && (
                  <div className="p-4 rounded-2xl bg-white ring-1 ring-zinc-200 flex flex-col gap-2">
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-black text-zinc-500 uppercase tracking-widest">정산</span>
                      <span className="text-xs text-zinc-400">{entry.settlement.settledPeriod}</span>
                    </div>
                    ...
                    {entry.settlement.unusedAmount > 0 && (() => {
                      ...
                      return context ? (
                        <p className="text-xs text-zinc-500 bg-zinc-50 rounded-lg p-3 leading-relaxed">
                          관련 모니터링: “{context}”
                        </p>
                      ) : (
                        <span className="text-[11px] text-amber-700 bg-amber-50 rounded-lg px-2.5 py-1 self-start">
                          미사용 이유 확인 필요
                        </span>
                      )
                    })()}
                ...
                {entry.kind === 'review' && entry.review && (
                  <div className="p-4 rounded-2xl bg-zinc-50 ring-1 ring-zinc-200 flex flex-col gap-1">
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-bold text-zinc-500">심의 {DECISION_LABEL[entry.review.decision] ?? entry.review.decision}</span>
                      <span className="text-xs text-zinc-400">{entry.review.reviewDate}</span>
                    </div>
                    {entry.review.reason && (
                      <p className="text-sm text-zinc-600 leading-relaxed">사유: {entry.review.reason}</p>
                    )}
                  </div>
                )}
              </li>
