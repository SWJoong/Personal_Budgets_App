...
const TYPE_STYLE: Record<string, string> = {
  신청서: 'bg-sky-50 text-sky-700',
  동의서: 'bg-emerald-50 text-emerald-700',
...
export default function DocumentShelfClient({ shelf }: { shelf: DocumentShelf }) {
  ...
  if (shelf.participants.length === 0) {
    return <p className="text-sm text-zinc-500 leading-relaxed py-12 text-center">아직 등록된 서류가 없어요.</p>
  }

  return (
    <div className="flex flex-col gap-4">
      <section className="p-4 rounded-2xl bg-zinc-50 ring-1 ring-zinc-200">
          ...
          return (
            <li key={p.participantId} className="rounded-2xl bg-white ring-1 ring-zinc-200 overflow-hidden">
              <button
                ...
                className="w-full flex items-center justify-between gap-2 p-4 min-h-[44px] text-left hover:bg-zinc-50 transition-colors"
              >
                <span className="font-bold text-zinc-800 truncate">{p.participantName}</span>
              ...
              {isOpen && (
                <ul className="border-t border-zinc-100">
                  {p.docs.map((d) => (
                    <li
                      key={d.id}
                      className="flex items-center justify-between gap-3 px-4 py-3 border-t border-zinc-50 first:border-t-0"
                    >
                      <div className="flex flex-col min-w-0 gap-1">
                        <div className="flex items-center gap-2">
                          <span
                            className={`text-[11px] font-bold px-2 py-0.5 rounded-full shrink-0 ${TYPE_STYLE[d.docTypeLabel] ?? 'bg-zinc-100 text-zinc-600'}`}
                          >
                            {d.docTypeLabel}
                          </span>
                          <span className="text-sm truncate">{d.fileName}</span>
                        </div>
                        <span className="text-xs text-zinc-500">{d.createdAt.slice(0, 10)}</span>
                        {d.note && <span className="text-xs text-zinc-400 leading-relaxed">{d.note}</span>}
                      </div>
                      <button
                        onClick={() => handleOpen(d.id)}
                        disabled={pending}
                        className="shrink-0 px-3 min-h-[44px] rounded-xl bg-zinc-900 text-white text-sm font-bold hover:bg-zinc-800 disabled:opacity-50"
                      >
                        열기
                      </button>
                    </li>
