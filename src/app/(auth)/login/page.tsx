...
// ────────────────────────────────────────────────────────────────
// 데모 로그인 — 실제로 시드된 Supabase Auth 계정에 로그인한다
// (쿠키를 직접 쓰는 스푸핑이 아니다. 일반 로그인과 동일한 RLS 를 탄다.)
// ────────────────────────────────────────────────────────────────
...
function DemoLoginSection() {
  ...
  const roles: { role: DemoRole; label: string; emoji: string }[] = [
    { role: "admin", label: "관리자로 둘러보기", emoji: "👨‍💼" },
    { role: "supporter", label: "담당자로 둘러보기", emoji: "🧑‍💻" },
    { role: "participant", label: "당사자로 둘러보기", emoji: "🙋" },
  ];

  const handleClick = (role: DemoRole) => {
    setError("");
    setPendingRole(role);
    ...
    <>
        ...
        <div className="flex w-full max-w-md flex-col gap-8 rounded-3xl bg-card p-8 md:p-12 shadow-xl ring-1 ring-border">
          ...
          <div className="flex flex-col items-center gap-4 text-center">
            <button
              ...
              className="w-24 h-24 rounded-3xl bg-primary flex items-center justify-center text-5xl shadow-lg hover:scale-105 transition-transform"
              aria-label="로고"
            >
              💰
            </button>
          ...
          <button
            ...
            disabled={loading}
            className="flex items-center justify-center gap-3 w-full py-4 px-6 rounded-2xl border-2 border-border bg-card hover:bg-muted-hover hover:border-border transition-all font-bold text-foreground text-base shadow-sm disabled:opacity-60 disabled:cursor-not-allowed min-h-[56px]"
          >
            {loading ? (
          ...
          {/* 데모 로그인 (NEXT_PUBLIC_DEMO_LOGIN_ENABLED=true 일 때만 노출) */}
          ...
          <div className="rounded-2xl border border-border overflow-hidden">
            <button
              ...
              className="w-full flex items-center justify-between px-5 py-3.5 text-sm font-medium text-muted-foreground hover:bg-muted-hover hover:text-foreground transition-colors"
            >
            ...
            {infoOpen && (
              <div className="px-5 pb-5 pt-1 text-sm text-foreground space-y-5 border-t border-border">
                ...
                <a
                  ...
                  className="flex items-center justify-center gap-2 w-full py-2.5 rounded-xl border border-border text-xs font-medium text-foreground hover:bg-muted-hover transition-colors"
                >
      ...
      >
        ...
        <button
          ...
          className="w-full py-2 rounded-xl bg-hero text-hero-foreground text-sm font-bold hover:bg-hero-hover transition-colors"
        >
          닫기
        </button>
      </Modal>
...
// ────────────────────────────────────────────────────────────────
// 페이지 진입점 — 구글 로그인 + (활성화 시) 데모 로그인을 함께 보여준다
// ────────────────────────────────────────────────────────────────
