  ...
  return (
    ...
    >
      {Array.from({ length: total }).map((_, i) => (
        <span key={i} className="text-lg leading-none" aria-hidden="true">
          {i < done ? '🟢' : '⭕'}
        </span>
      ))}
    </div>
...
export default function MonthlyPlanEasyCard({
  ...
  return (
    <div className="flex flex-col items-center gap-2 bg-white rounded-2xl ring-1 ring-zinc-200 shadow-sm p-4 min-w-[140px] max-w-[180px]">
      {/* 이미지 또는 이모지 fallback */}
      {easyImageUrl ? (
        <img
          src={easyImageUrl}
        ...
        <div
          className={`w-20 h-20 rounded-xl flex items-center justify-center text-4xl ${fallback.bg}`}
