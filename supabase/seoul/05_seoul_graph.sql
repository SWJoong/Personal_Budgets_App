-- =====================================================================
-- 05. 그래프 오버레이 — 파생 우선 방식 (외래키 → 트리플 투영)
--
-- 원본: Plan&Source/ontology/seoul/seoul_graph_overlay.sql
-- 대응 온톨로지: Plan&Source/ontology/seoul/seoul_ontology.rdf (술어 이름 1:1)
--
-- 이 파일이 답하는 질문: "기존 Supabase 로도 온톨로지형 DB 가 되는가?" — 된다.
-- 그래프 DB 를 따로 두지 않고, 이미 있는 외래키를 트리플(주어-술어-목적어)로
-- 투영한다(derived-first). 전체 근거·한계는 원본 파일 §6·§7과
-- Plan&Source/서울형_온톨로지_설계_v1.md §5-1 참조.
--
-- ★ 모든 뷰가 WITH (security_invoker = true) 를 반드시 가져야 한다.
--   PostgreSQL 뷰는 기본값이 "소유자 권한 실행"이라, 이 옵션이 없으면
--   하위 테이블의 RLS 가 통째로 우회된다 — 실제로 이 초안에서 한 번 발견된 결함이다.
--   PostgreSQL 15 이상 필요.
--
-- ★ 이번에 함께 고친 것: v_seoul_graph_nodes 가 참여자 이름을 표시할 때
--   profiles.id = participants.id 로 조인했다. 이는 로그인 계정과 참여자를
--   같은 것으로 취급하는, 이 세션에서 고치고 있는 바로 그 가정이다.
--   participants.name(자체 보유) 을 우선하고, profiles 는 auth_user_id 로
--   조인하도록 고쳤다.
-- =====================================================================
...
CREATE OR REPLACE VIEW public.v_seoul_graph_nodes
  ...
  SELECT 'Participant'::TEXT AS node_type, p.id,
         COALESCE(p.name, pr.name, '이름없음') AS label
