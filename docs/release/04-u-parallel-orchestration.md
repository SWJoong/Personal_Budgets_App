...
세션 ID는 인스턴스마다 바뀌므로 **매번 발견**한다. `mcp__ccd_session_mgmt__list_sessions`로 나열 후
`cwd == 이 저장소` + 제목 패턴으로 역할 판별:

| 역할 | 발견 기준 | 예(2026-09-04 관측) |
|---|---|---|
| **W**(설계·검증) | cwd=저장소 · 제목 "W측…"·PL/QA성 | `local_47ed2b3b` "W측 작업 진행"(로컬·idle) + Windows Remote Control("PRD 정합성 평가" 등, offline) |
| **U**(구현·배포) | cwd=저장소 · 제목 "U측…" | 이 세션(오케스트레이터·live) + "U측 잔여 task 확인"·"U측 대기 중 작업 점검" |
| 무관 | cwd≠저장소 | LLM_SOCIALWORK_WIKI·EASYREAD·lecture_workspace 등 — 조율 대상 아님 |

`ListAgents`는 **지금 살아있는(메시지 가능한)** 세션만 보여 준다 — 라이브 메시징 대상은 여기서 고른다.
...
| **세션 핸드오프** | `mcp__…__send_message` | 상대 세션에 **user turn**으로 도착 | **idle 로컬 세션 깨워 인계**(예: W측에 검증 큐 전달). 실행 안 중인 대화형 세션도 다음 열람 때 봄 |

### 8-3. 절차
1. **발견**: `list_sessions`(레지스트리) + `ListAgents`(라이브 여부).
2. **핸드오프**: W가 live면 `SendMessage`, idle면 `send_message`로 **검증 큐/상태를 정보 전달**.
   코드는 언제나 PR, 정본 상태는 언제나 agent-sync에도 남긴다(교차 메시지는 보조).
3. **워커 조율**: 서브에이전트는 완료 알림으로 취합(§7). 필요 시 `SendMessage`로 후속 지시.
