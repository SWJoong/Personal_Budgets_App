...
- **하단엔 이 버튼 하나만**(사용자 요구 2026-08-21): 기존 `TabBar`(🏠홈·🧾영수증·⚙더보기)를 걷어내고
  **중앙 FAB `📷 내가 쓴 돈 적기` 단 하나**만 하단에 둔다. 양옆 홈·더보기 탭 없음. 홈은 기본 화면이라
  별도 버튼 불필요, **더보기(설정·기타 메뉴)는 상단 헤더(⚙)로 이관**한다(네비 손실 방지).
...
- **하단 FAB**(§6, 사용자 요구): 하단엔 **단일 중앙 FAB `📷 내가 쓴 돈 적기` 하나만**(홈·더보기 탭 제거).
  카메라 우선 진입 → 옛 `analyzeReceipt` OCR 재사용 → 지출 저장. 원형 64px·`pb-safe`·라벨 노출.
  더보기(설정 등)는 **상단 헤더 ⚙**로 이관. `TabBar` 는 당사자 뷰에서 이 FAB 로 대체(지원자/관리자 탭바는 유지).
