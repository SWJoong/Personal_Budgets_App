...
> **★ 제거됨(REMOVED) 2026-09-19** — 사용자 `/goal` 결정으로 관계망(Track B) 기능 전체가 앱에서
> 제거되었다(PR #171 squash `7ce7c72`, 후속 주석 tidy #172). **사유**: 온톨로지 구축 미성숙 + 서울형
> 개인예산제 특수성 → 앱을 3축(당사자 자기주도성 · 실무자 행정간소화/계획공유 · 관리자 파악/슈퍼비전)에
> 집중. 관계망은 온톨로지 성숙을 전제로 한 파생그래프 큐레이션이라 아직 이르다는 판단.
> 대상 구현·빌드SQL·verify 계약 모두 삭제, 라이브 DB 객체도 드롭·검증 완료(실행노트 `docs/release/13`).
> 이 문서는 **역사적 설계 기록**으로 보존한다(향후 재도입 시 출발점). 아래 본문은 제거 이전 시점 설계다.
