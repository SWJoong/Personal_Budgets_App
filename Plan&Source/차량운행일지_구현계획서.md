...
> **최종 업데이트**: 2026-03-13 21:00 | **전체 진행률**: Phase 1~34 완료 ✅ | 서비스 운영 중 (86개 기관)
...
- 기관 신청 목록 (탭: 대기 중 / 거절됨, 신청 정보 + AI 검증 결과 표시)
  ...
  - 거절됨: 내역 조회 + 제거 가능
- 기관 관리 (승인된 기관 목록, 기관명·주소 인라인 편집)
- 사용자 피드백 관리 (피드백 목록 조회/상태 관리)
- 알림 관리

### 6.5 공통 공개 페이지
- **FAQ 페이지** (`FAQPage.tsx`): 자주 묻는 질문 아코디언 UI, 대시보드 우측 배치
- **업데이트 소식** (`ReleaseNotesPage.tsx`): 서비스 변경 이력을 사용자 친화적으로 표시
...
- **차량 예약**: 달력 보기, 예약 관리
- **설정**: 기관 정보(기관명·주소는 읽기 전용, 이메일·전화번호 편집 가능), 결재란 항목 설정, 공휴일 관리, 푸시 알림 설정, 다크 모드 토글
...
|--------|------|-----------|--------|--------|------|
| `reservationReminder` | `reservationReminder.ts` | 5분마다 | KST | 0회 | ① 예약 시작 10분 전 FCM 알림 ② 운행 종료 후 일지 미작성 FCM 알림 |
...
| `migrateCustomClaims` | `migrateCustomClaims.ts` | asia-northeast3 | ✅ (x-migrate-key 헤더) | — | 기존 사용자 전체 Custom Claims 일괄 설정 (1회성) |
...
│   │   │   ├── ReportCharts.tsx           통계 보고서 차트 컴포넌트 (Recharts)
...
│   ├── contexts/                     ← React 컨텍스트 (3개)
│   │   ├── ThemeContext.tsx              다크/라이트 모드 관리 (개인 설정 Firestore 저장)
│   │   ├── FontSizeContext.tsx           글꼴 크기 조절 (소/중/대, localStorage 저장)
│   │   └── ConfirmContext.tsx            Promise 기반 커스텀 confirm 모달 컨텍스트
│   ├── hooks/                        ← 커스텀 훅 (로직 분리)
...
│   │   ├── useToast.tsx                  토스트 메시지 훅 (성공/에러/경고)
...
│       ├── excelExport.ts                운행일지 + 주유 기록 엑셀 다운로드
...
│   │   ├── migrateCustomClaims.ts        기존 사용자 Custom Claims 일괄 마이그레이션 (HTTP)
...
│   ├── package.json
│   └── lib/                              빌드 출력 디렉토리
├── e2e/                              ← E2E 테스트 (Playwright, TypeScript)
│   ├── accessibility.spec.ts             접근성 테스트
│   ├── landing.spec.ts                   랜딩 페이지 테스트 (8개)
...
│   └── terms-privacy.spec.ts             약관/개인정보 테스트 (5개)
